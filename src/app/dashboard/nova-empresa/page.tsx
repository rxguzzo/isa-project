  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessage(
          `Erro: ${data.message || "Não foi possível cadastrar a empresa."}`
        );
    ...
    <div className="grid min-h-screen w-full lg:grid-cols-2">
      ...
      >
        <div className="relative z-10">
          ...
          <p className="mt-4 max-w-sm text-lg text-gray-600">
            Adicione uma nova empresa para gerenciar suas demandas e otimizar
            processos.
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center bg-white p-6 sm:p-12 animate-slide-in-right">
        <div className="w-full max-w-md">
          <Link
            href="/dashboard"
            className="mb-6 inline-flex items-center text-sm text-gray-500 hover:text-[#b91c1c]"
          >
            <ArrowLeft className="h-4 w-4 mr-2" />
            Voltar para o seletor de empresas
          </Link>
          <h1 className="text-3xl font-bold mb-2 text-gray-800 font-display">
            Cadastrar Empresa
          </h1>
