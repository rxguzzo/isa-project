...
// Estado inicial para o formulário com valor padrão para nivelUrgencia
const initialState = {
  areaDemanda: [] as string[],
  assunto: '',
  descricao: '',
  objetivos: '',
  nivelUrgencia: 'media', // <-- CORREÇÃO AQUI: Definido um valor padrão
  orcamento: '',
  comoConheceu: '',
...
export default function NovaDemandaPage() {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (!formState.assunto.trim() || !formState.descricao.trim() || !formState.nivelUrgencia) {
      setFormStatus('Erro: Assunto, Descrição e Nível de Urgência são obrigatórios.');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        router.refresh();
        setTimeout(() => {
          router.push(`/dashboard/${empresaId}/demandas`);
        }, 2000);
      } else {
          ...
          <form onSubmit={handleSubmit} className="space-y-10">
            <fieldset>
              ...
              <div className="space-y-6">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Área da Demanda*</label>
                ...
                <div>
                  <label htmlFor="objetivos" className="block text-sm font-medium text-gray-700">Objetivos Esperados</label>
